...
public class BloodDecayHandler {
    ...
    private static Holder<MobEffect> getGarlicEffect() {
        return BuiltInRegistries.MOB_EFFECT.getHolder(ResourceLocation.fromNamespaceAndPath("vampirism", "garlic")).orElse(null);
    }
    ...
    public static void onLivingTick(EntityTickEvent.Post event) {
        ...
        VampirismAPI.getExtendedCreatureVampirism(maid).ifPresent(ext -> {
                ...
                // Vampirism returns -1 when entity type is not registered in blood data map
                // In this case, skip all blood decay processing
                if (currentBlood < 0 || maxBlood <= 0) {
                    return;
                }

                // === 反自动恢复 ===
                int lastKnown = cap.getLastKnownBlood();
                if (lastKnown <= 0 && currentBlood > 0) {
                    // 首次初始化（lastKnown为0且当前有血量）
                ...
                // === 大蒜效果检测 ===
                Holder<MobEffect> garlicEffect = getGarlicEffect();
    ...
    public static void onMobEffectApplicable(MobEffectEvent.Applicable event) {
        ...
        VampireMaidCapability effectCap = entity.getData(ModAttachments.VAMPIRE_MAID.get());
        if (effectCap.isVampire()) {
            var effect = event.getEffectInstance().getEffect();
            if (effect == MobEffects.WEAKNESS || (effect == MobEffects.MOVEMENT_SLOWDOWN && !effectCap.isApplyingSlowness())) {
    ...
    public static void onLivingAttack(LivingIncomingDamageEvent event) {
        ...
        VampireMaidCapability attackCap = entity.getData(ModAttachments.VAMPIRE_MAID.get());
        if (!attackCap.isVampire()) {
            return;
        }
