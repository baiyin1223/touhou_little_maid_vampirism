...
public class MaidTrainingMenu extends AbstractContainerMenu {
    ...
    public MaidTrainingMenu(int containerId, Inventory playerInventory, BlockPos trainerPos) {
        ...
        // Top row: iron(27,26), gold(57,26), vampBook(86,26)
        // Bottom row: soulOrb(42,50), pureBlood(71,50)
    ...
    private int countVampBook() {
        var item = BuiltInRegistries.ITEM.get(new ResourceLocation("vampirism", "vampire_book"));
        return item == null ? 0 : count(item);
    }

    private int countSoulOrb() {
        var item = BuiltInRegistries.ITEM.get(new ResourceLocation("vampirism", "soul_orb_vampire"));
        return item == null ? 0 : count(item);
    }
