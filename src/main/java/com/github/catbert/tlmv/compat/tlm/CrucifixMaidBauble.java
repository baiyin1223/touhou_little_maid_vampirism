...
/**
 * 猎人女仆十字架饰品——每7秒脉冲一次，持续3秒推远周围亡灵/吸血鬼。
 */
public class CrucifixMaidBauble implements IMaidBauble {

    private static final int CYCLE_TOTAL = 35;  // 15 active + 20 cooldown (bauble ticks @ 5Hz)
    private static final int ACTIVE_DURATION = 15; // 15 bauble ticks = 3 seconds
    ...
    public void onTick(EntityMaid maid, ItemStack baubleItem) {
        ...
        if (maid.tickCount % 4 != 0) return; // 5Hz

        var cap = maid.getData(ModAttachments.VAMPIRE_MAID.get());
        if (!cap.isHunter() || cap.getHunterLevel() < minHunterLevel) return;

        // Cycle timer: active 3s, cooldown 4s
        UUID id = maid.getUUID();
        int timer = cycleTimers.getOrDefault(id, 0);
        timer = (timer + 1) % CYCLE_TOTAL;
        cycleTimers.put(id, timer);
        if (timer >= ACTIVE_DURATION) return; // cooldown phase
        ...
        for (LivingEntity target : entities) {
            // Push entity position directly (AI overrides velocity, so move coordinates)
            ...
            // Also nudge velocity for visual feedback
    ...
    private double getPushStrength(int entityTier) {
        return switch (minHunterLevel) {
            case 1 -> entityTier > 1 ? 0.05 : 0.15; // NORMAL
            case 3 -> entityTier > 2 ? 0.05 : 0.20; // ENHANCED
            default -> entityTier > 3 ? 0.10 : 0.25; // ULTIMATE
        };
    }

    private boolean affectsEntity(LivingEntity e) {
        return e.getType().is(EntityTypeTags.UNDEAD) || isVampire(e);
    }
    ...
    private static int determineEntityTier(LivingEntity e) {
        try {
            if (e instanceof Player) {
                ...
                // CrucifixResistant refinement increases tier
    ...
    private double determineSlowdown(int entityTier) {
        return switch (minHunterLevel) {
            case 1 -> entityTier > 1 ? 0.1 : 0.5; // NORMAL
            case 3 -> entityTier > 2 ? 0.1 : 0.5; // ENHANCED
            default -> entityTier > 3 ? 0.3 : 0.5; // ULTIMATE (minHunterLevel=5)
        };
    }
}
