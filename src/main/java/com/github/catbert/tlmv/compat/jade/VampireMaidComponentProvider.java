...
public enum VampireMaidComponentProvider implements IEntityComponentProvider, IServerDataProvider<EntityAccessor> {
    ...
    private static final ResourceLocation ID = new ResourceLocation(TLMVMain.MOD_ID, "vampire_status");
                ...
                if (isVampire) {
                    ...
                    if (entity instanceof PathfinderMob mob) {
                        VampirismAPI.getExtendedCreatureVampirism(mob).ifPresent(ext -> {
                            data.putInt("bloodLevel", ext.getBlood());
                            data.putInt("maxBlood", ext.getMaxBlood());
                        });
                    }
                }
            }
